...
// todo figure out why this is returning as obselete
describe('SingleRecipe', () => {
  ...
  it('renders without crashing', () => {
    const wrapper = shallow(<SingleRecipe results={results} initialIndex={0}/>)
    expect(wrapper).toBeDefined()
  })
  it('goes to next recipe', () => {
    const wrapper = shallow(<SingleRecipe results={results} initialIndex={0}/>)
    const next_button = wrapper.find("#next-recipe")    
    for(var i = 0; i <= results.length + 1; i++) {
      wrapper.find("#next-recipe").props().onClick()
    }
    // wrapper.find("#next-recipe").simulate("click")
  })
  // TODO test the results
  it('goes to previous recipe', () => {
    const wrapper = shallow(<SingleRecipe results={results} initialIndex={0}/>)
    const prev_button = wrapper.find("#prev-recipe")
    
    for(var i = 0; i <= results.length + 1; i++) {
      wrapper.find("#prev-recipe").props().onClick()
    }
  })

  test('has a valid snapshot', () => {
    const component = renderer.create(
    <SingleRecipe results={results} initialIndex={0}/>
    );
